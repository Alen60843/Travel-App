...
`buildContextBundle({ runId, subject, repositoryContext, diagnosis, relevantMemory })` is pure. It performs no source loading, filesystem or Git access, state or Memory writes, process execution, provider calls, clock access, or randomness. Scope validation fails closed when the diagnosis or relevant-memory query conflicts with the requested run or exact structured subject. A relevant-memory result without `runId` is intentionally accepted for exact-subject cross-run history.
