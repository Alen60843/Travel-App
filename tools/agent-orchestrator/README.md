...
For Claude CLI schema enforcement and the one-time continuation for an already-consumed prompt-only review retry, see [Claude structured review output](CLAUDE_STRUCTURED_REVIEW_OUTPUT.md).
...
The detected development interfaces were Codex `0.149.0-alpha.4.1` (`codex exec`) and Claude Code `2.1.71` (`claude --print`). Codex supports explicit cwd, sandbox/approval policy, structured-output files, and `-m/--model` (confirmed via `codex exec --help`), but documents no effort/reasoning flag. Claude's local help lists `default`, `dontAsk`, `plan`, `acceptEdits`, `bypassPermissions`, and `auto` as distinct permission modes, plus non-interactive output, JSON schemas, `--model`, and `--effort`; `extra_high` maps to Claude's documented `xhigh`. Read-only Claude tasks use `dontAsk` for headless workflow and independently restrict tools to `Read,Glob,Grep`; they do not enter Plan Mode. Claude review-like roles use `--output-format json --json-schema ...` and extract only the verified `structured_output` field from the successful result envelope before the existing strict review parser runs. Unsupported flags are never fabricated. `agents:plan` prints each resolved executable's path, discovery source, and per-agent model/effort override support so this stays visible per run rather than only documented here.
...
pnpm agents:continue-claude-review-output <runId> <taskId>
...
`agents:retry-agent` = failed process + **clean** worktree. It is an explicit recovery authorization for one task whose process failed before any structured result was accepted. It accepts only a terminal `FAILED`/`BLOCKED` run with untouched integration state, a `FAILED` task whose final attempt ended as `AGENT_FAILED` or `AGENT_TIMEOUT`, satisfied dependencies, no task commit or accepted handoff/review, and a clean registered worktree still at its prepared SHA. It archives the terminal failure metadata append-only, makes only that task `READY`, resets only pristine `TASK_DEPENDENCY_FAILED` descendants attributable solely to it, and makes the run resumable. It never parses provider error prose and never invokes an agent; use `agents:resume` afterward. Successful tasks and their attempts/commits are immutable. A *dirty* worktree with the same failure codes is `agents:salvage-task`'s domain instead — the two commands are mutually exclusive on the same task state.

`agents:retry-review-output` authorizes one fresh normal attempt for a static `review` or `final_review` task that is `FAILED / REVIEW_BLOCKED` after its provider process succeeded but no review was accepted. It requires a quiescent terminal run, untouched integration, a read-only task, an unchanged applicable condition and review-lineage budget, satisfied unchanged dependencies, and a clean registered worktree at its exact prepared HEAD. The command records an append-only `reviewOutputRecoveries` entry containing the original error, attempt, stdout path and SHA-256, plus reopened descendants; the original attempt, stdout, failed repair evidence, and run errors remain unchanged. It then makes only the review `READY`, reopens only pristine solely-attributable `TASK_DEPENDENCY_FAILED` descendants, and sets the run to `RUNNING`. Authorization invokes no provider. `agents:resume` performs the retry through the ordinary strict review parser; prose remains invalid. The v2 budget is exactly one authorization per review round.

`agents:continue-claude-review-output` does not widen that ordinary budget. It is available only after the same static Claude review round has two consecutive successful provider processes with prompt-only prose output and the first is already bound to the consumed v2 retry. The append-only v3 record hashes both stdout artifacts, the consumed recovery, old/new Claude adapter contract identities, prepared HEAD, dependencies, accepted reviews, and prompt inputs. Authorization makes no provider call; one later `agents:resume` may make exactly one post-contract-fix invocation. See [Claude structured review output](CLAUDE_STRUCTURED_REVIEW_OUTPUT.md).

`agents:salvage-task` = failed/timed-out writer + useful **dirty** worktree + deterministic verification — the inverse case from `agents:retry-agent`. It accepts only a `FAILED`/`BLOCKED` task whose final attempt ended `AGENT_TIMEOUT` or `AGENT_FAILED`, no task commit, satisfied dependencies, and a dirty worktree whose every changed tracked file is inside the task's own ownership globs, with no foreign commits, no unexpected untracked files, and a clean `git diff --check`. Like `retry-agent`, the eligibility class is derived only from the persisted error code and final attempt outcome — never from provider stderr/error prose, a model name, or any vendor-specific text ("usage limit" included); the diff itself must still prove safe through every check above regardless of which of the two failed the attempt. A dirty diff is never treated as success on its own: it runs `agentWorktree.prepare` (if configured, environment readiness only) and then the phase's `salvage.verify` commands — a config surface categorically separate from `prepare` and required for any task to be salvageable at all, since a passing test claim in agent stdout or a human's manual run is never sufficient evidence by itself. A `salvage.verify` command that mutates tracked source fails the salvage closed regardless of its own exit code. Verification success is checkpointed bound to the exact worktree HEAD, diff content, and verify config that produced it, so a crash-resumed salvage only reuses a still-valid checkpoint. The Orchestrator itself then creates the commit, the same way `agents:apply-integration-fix` does — salvage code never calls `git commit`. It then resets only pristine `TASK_DEPENDENCY_FAILED` descendants attributable solely to that task and makes the run resumable, the same way `agents:retry-agent` does. A task with a required canonical finding still needs one bounded, evidence-only repair call (the same cascade `agents:recover-handoffs` uses) to complete its `findingResponses`; salvage never bypasses canonical validation. Use `agents:resume` afterward. See `docs/superpowers/specs/2026-09-04-orchestrator-recovery-hardening-design.md` for the original design and its later revision note extending eligibility to `AGENT_FAILED`.

`agents:verify-blocked-task <runId> <taskId>` explicitly recovers a preserved writer whose process succeeded but whose valid handoff reports `blocked` (`REVIEW_BLOCKED`). Run it from the host terminal: it reuses the phase's required `salvage.verify` commands (including an already-authorized recovery-policy overlay) in the preserved task worktree, with `agentWorktree.prepare` first if configured. It accepts no shell-command argument and invokes no provider. HEAD and all commit-eligible dirty content must remain unchanged through preparation and verification; ignored generated artifacts are allowed. Review/debate tasks, adaptive runs, dependency drift, moved HEAD, existing commits, ownership violations, and started integration are refused.

Passing verification creates one canonical task commit, sets the task to `SUCCEEDED` and run to `RUNNING`, and reopens only dependency failures attributable solely to that task. The original blocked handoff stays byte-for-byte intact. A separate completion handoff retains its decisions, risks, and original test records, adding truthful host verification results with logs, exit codes, and timings. Recovery uses salvage checkpoints bound to HEAD, dirty content, commands, and original evidence. A repeated invocation after success refuses without another commit or history entry; a crash before commit can reuse matching verification, while an unrecorded commit after a crash fails closed for inspection, as in salvage. `agents:resume` alone never promotes this blocked task. After successful recovery, run `pnpm agents:resume <runId>` to continue the same run to review.

`pnpm agents:retry-preflight <run-id> <task-id>` reconsiders only a **static, read-only review-budget guard failure before any agent invocation**. The run must be `FAILED` or `BLOCKED` with no `PENDING`, `READY`, or `RUNNING` tasks, and integration must not have started or acquired recovery state. The target must be a terminal `review`, `final_review`, or `synthesis` task with `BLOCKED_FOR_HUMAN_REVIEW`, zero `agentAttempts`, zero `reviewRounds`, and no commit, accepted output, repair history, or salvage evidence. Dependencies must remain satisfied. The normal condition evaluator must say run (a condition that now skips is refused), and the production `completedReviewRounds` calculation plus strict `assertReviewRoundAllowed` must pass. Historical error prose and historical round totals do not authorize recovery. Other errors, provider failures, cancellation, writers, and adaptive runs are excluded.

An unprepared task must have no orphaned directory, branch, registration, or partial checkpoint. An already prepared worktree is **retained and reused** through the existing `prepareTask` path: registration, run/task identity, branch, immutable base, HEAD, cleanliness (including ordinary untracked files), completed preparation, and the ordered canonical dependency cherry-picks and patches must all match. Ignored bootstrap outputs are retained. Nothing is deleted, reset, cherry-picked, or prepared by the recovery command; normal resume neither creates a duplicate worktree nor reruns a reusable preparation checkpoint.

Only pristine dependency-failure descendants attributable solely to the target reopen, using the existing dependency-descendant rules; execution evidence causes refusal. A single atomic state write restores scheduler-derived `READY`/`PENDING` states and sets the run to `RUNNING`. Historical run errors and events remain intact. Before this write, `PREFLIGHT_RETRY_AUTHORIZED` durably archives the complete previous task (including failure timestamps), previous run status, current round eligibility, and reopened IDs. The event records authorization, not proof that the state write completed: after a crash before that write, repeat `retry-preflight`; after it, use normal resume. Prepared worktrees survive either window unchanged. A per-run lock serializes concurrent preflight retries; dead-owner locks are recoverable (an empty lock from a creation-time crash has a 30-second grace period). Like other explicit recovery commands, operate on a quiescent run without concurrent resume or other recovery commands. This command also rejects persisted state changes detected during eligibility checks.

The command invokes no provider and does not execute the task. Inspect its JSON result, then run `pnpm agents:resume <run-id>` to perform the authorized work. This is not a generic retry or force-state interface.
...
Review and correction agents receive the actual dependency diff (bounded to 2 MiB so oversized tasks must be split), implementation handoffs, and earlier findings. Review, final-review, and escalation roles must start from that diff and the explicit task invariants; they inspect additional files only to prove or disprove a concrete suspicion using directly relevant schema, contracts, or neighboring code. Their contract forbids broad speculative repository exploration, unrelated-module review, architecture rediscovery, and mentally re-implementing the task. This preserves adversarial depth while requiring a material evidence-backed result within the configured execution budget. Correction prompts require each finding to be marked CONFIRMED or REJECTED with evidence. Review/correction lineages are bounded by `maxReviewRounds`; unresolved material findings stop as `BLOCKED_FOR_HUMAN_REVIEW`.

Review-budget audit: the guard previously counted all successful `review`, `final_review`, and `synthesis` ancestors. That accidentally charged later workstreams for earlier features' reviews. The Phase 5 example uses an unconditioned review → correction → final-review chain; Phase 5 real, Phase 6, and generated solver/verifier tasks also use `condition.reviewOf` to connect correction and final review to their initial review. Both forms represent one lineage. With `maxReviewRounds: 2`, its initial and final review are allowed; once both succeed, a third review in that lineage is blocked before agent invocation.

Lineage is derived from the validated DAG: `condition.reviewOf` selects the referenced task's lineage; otherwise dependencies carry lineage through correction and other intermediate tasks. An `implementation` task starts new work and stops inheritance of upstream review lineages. A review with no inherited lineage roots its own. Unqualified joins, including synthesis over multiple review inputs, conservatively retain all input lineages; successful matching ancestors are counted once each. Explicit links back to the initial review still count intervening successful re-reviews in that lineage. Task names and file prefixes have no role. Skipped tasks consume no rounds, and conditions are evaluated before the guard. Debate and escalation are not review rounds. Adaptive tasks use this same calculation, with `maxReviewRounds` still derived from `maxTotalWorkUnits`; canonical correction rounds remain independently bounded by `correctionPolicy.maxRounds`.

Persisted `reviewRounds` remains task-local: live successful review completion increments it, as does crash reconciliation of a recovered review verdict (including a recovered blocked verdict). It is not a lineage total or the number of provider attempts. `agentAttempts` records actual invocations; guard diagnostics `completedRounds` count successful review tasks in the current lineage, and `REVIEW_STARTED.data.round` is that count plus one. Thus a final review normally has one attempt and `reviewRounds: 1`, but emits lineage round 2. A genuinely exhausted third review can have zero attempts and `reviewRounds: 0` with `completedRounds: 2`. Existing config and state need no migration. A persisted pre-invocation `BLOCKED_FOR_HUMAN_REVIEW` failure requires explicit `agents:retry-preflight` authorization when the current guard now permits it; normal resume alone still does not reopen it.
...
- **Claude** (`claude --help`, reverified against the currently installed Claude Code 2.1.71): `--model <model>` is a real, independent flag alongside the existing `--effort`. Wired in `agents/claude-agent.ts`, added to the invocation only when a task requests one.
