...
**Revision note (2026-09-10 addendum, `orchestrator/dirty-agent-failure-salvage`):** a real Phase 7 dogfood run (`phase7-chat-realtime-transport`) hit exactly the case the eligibility rule below (point 1 in Part B's acceptance criteria) deliberately excluded: a writer's process ended `AGENT_FAILED` (a reported provider usage limit, never parsed as such) after leaving a correct, evidence-backed dirty diff. `checkSalvageEligibility` now accepts a completed final attempt of *either* `AGENT_TIMEOUT` or `AGENT_FAILED` as the same single eligibility class — derived only from the persisted error code and attempt outcome, never from provider stderr/error prose — under the renamed reason code `SALVAGE_ATTEMPT_NOT_ELIGIBLE` (`SALVAGE_NOT_TIMED_OUT` no longer describes what it guards). Every other invariant below (ownership, foreign commits, diff-check, required `salvage.verify`, checkpoint binding, commit-order safety) is unchanged and applies identically regardless of which of the two failure codes ended the attempt. While adding regression coverage for this, plain (non-blocked-writer) `salvage-task` was found to never call `dependencyOnlyDescendantsToReopen` or flip the run's top-level status to `RUNNING` on success — unlike `retry-agent` and the blocked-writer verification path, a successfully salvaged task left the run permanently stuck on the very next `agents:resume`, and any descendant blocked solely by the salvaged task's original failure stayed `TASK_DEPENDENCY_FAILED` forever. This was a latent gap in the base salvage path added by Part B below (not a regression from this addendum) and is now fixed for both failure classes: `recoverDirtyWriter` computes `reopenedTasks` unconditionally, matching what the blocked-writer path already did.
